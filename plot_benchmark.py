...
def run_parallel_benchmarks(n_range, n_samples, config, n_workers=16):
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(tasks), desc="Benchmarking"):
...
def create_results_dataframe(results, base_csv_path):
    ...
    for N in sorted(pce_df['N'].unique()):
        exhaustive_mf = exhaustive_data[exhaustive_data['N'] == N]['MF_Exhaustive'].values
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark PCE Solver and generate plots")
    parser.add_argument('--ns', type=int, nargs='+', default=None, help="Specific N values to test")
    ...
    parser.add_argument('--output-csv', type=str, default='total_results.csv', help="Output CSV")
    parser.add_argument('--output-plot', type=str, default='benchmark_plot.png', help="Output plot")
    parser.add_argument('--base-csv', type=str, default='merged_results.csv', help="Base CSV")
    ...
    parser.add_argument('--n-restarts', type=int, default=50, help="Restarts per optimization")
    parser.add_argument('--maxiter', type=int, default=300, help="Max iterations per restart")
    parser.add_argument('--tabu-iterations', type=int, default=20000, help="Tabu search iterations")
